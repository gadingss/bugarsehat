                        ...
                        <div class="card-toolbar">
                            <div class="d-flex align-items-center gap-3">
                                <div class="position-relative">
                                    <span class="svg-icon svg-icon-1 position-absolute ms-4 translate-middle-y top-50"><i
                                            class="fas fa-search"></i></span>
                                    <input type="text" class="form-control form-control-solid w-250px ps-14"
                                        placeholder="Cari produk..." id="search-input" />
                                </div>
        ...
        <div class="row g-5 g-xl-6" id="products-container">
            @forelse($products as $product)
                <div class="col-12 col-md-6 col-lg-4 product-item" data-category="{{ $product->category }}"
                    data-promo="{{ $product->is_promo ? '1' : '0' }}" data-name="{{ strtolower($product->name) }}">
                    <div class="card card-flush h-100">
                        <div class="position-relative overflow-hidden rounded-top" style="height: 200px;">
                            <img src="{{ $product->image ? asset('storage/' . $product->image) : asset('metronic/assets/media/stock/600x400/img-1.jpg') }}"
                                 alt="{{ $product->name }}"
                                 class="w-100 h-100"
                                 style="object-fit: cover;"
                                 onerror="this.src='{{ asset('metronic/assets/media/stock/600x400/img-1.jpg') }}'">
                            @if($product->is_promo)
                                <div class="position-absolute top-0 end-0 m-3"><span
                                        class="badge badge-danger fs-7">PROMO</span></div>
                            @endif
                            @if($product->stock > 0 && $product->stock <= 5)
                                <div class="position-absolute top-0 start-0 m-3"><span class="badge badge-warning fs-7">Stok
                                        Terbatas</span></div>
                            @endif
                        </div>
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="mb-4">
                                @if($product->is_promo && $product->promo_price)
                                    <div class="d-flex align-items-center">
                                        <span class="fs-2 fw-bold text-primary me-2">Rp
                                            {{ number_format($product->promo_price, 0, ',', '.') }}</span>
                                        <span class="fs-6 text-gray-500 text-decoration-line-through">Rp
                                            {{ number_format($product->price, 0, ',', '.') }}</span>
                                    </div>
                                    <div class="text-success fs-8">Hemat Rp
                            ...
                            <div class="d-flex gap-2">
                                <button class="btn btn-light-primary flex-fill" onclick="viewProduct({{ $product->id }})"><i
                                        class="fas fa-eye me-2"></i>Detail</button>
                                @if($product->stock > 0)
                                    <button class="btn btn-primary flex-fill" onclick="addToCart({{ $product->id }})"><i
                                            class="fas fa-shopping-cart me-2"></i>Beli</button>
                                @else
...
@section('script')
    <script>
        ...
        function previewModalProductImage(input, productId) {
            if (input.files && input.files[0]) {
                const reader = new FileReader();
                reader.onload = function(e) {
                    document.getElementById('modal-product-preview-img-' + productId).src = e.target.result;
                    document.getElementById('modal-product-preview-' + productId).classList.remove('d-none');
                };
                reader.readAsDataURL(input.files[0]);
            }
        }

        function submitProductImage(event, productId) {
            event.preventDefault();
            const form = document.getElementById('upload-product-form-' + productId);
            ...
            .then(response => {
                if (response.redirected || response.ok) {
                    Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Gambar produk berhasil diperbarui.', timer: 2000, showConfirmButton: false });
                    // Reload modal content
                    viewProduct(productId);
                    // Reload page after short delay to update card images
            ...
            .catch(error => {
                ...
                Swal.fire({ icon: 'error', title: 'Gagal', text: error.message || 'Terjadi kesalahan saat upload.' });
                btn.disabled = false;
                btn.innerHTML = '<i class="ki-duotone ki-cloud-upload fs-5 me-1"><span class="path1"></span><span class="path2"></span></i> Simpan Gambar';
            });
        }
    </script>
@endsection