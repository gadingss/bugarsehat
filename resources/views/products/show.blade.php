                            ...
                            <div class="col-md-6">
                                ...
                                @else
                                    <div class="bg-light-primary rounded d-flex align-items-center justify-content-center"
                                        style="height: 400px;" id="product-image-preview-placeholder">
                                        <div class="text-center text-gray-400">
                                            <i class="ki-duotone ki-picture fs-3x mb-3"><span class="path1"></span><span
                                                    class="path2"></span></i>
                                ...
                                @if(auth()->user()->hasRole('User:Owner') || auth()->user()->hasRole('User:Staff'))
                                    {{-- Upload Gambar Form --}}
                                    <div class="mt-4 card border-dashed border-primary bg-light-primary">
                                        <div class="card-body p-4">
                                            <h6 class="fw-bold text-primary mb-3">
                                                <i class="ki-duotone ki-picture fs-5 me-1"><span class="path1"></span><span
                                                        class="path2"></span></i>
                                                {{ $product->image ? 'Ganti Gambar Produk' : 'Upload Gambar Produk' }}
                                            </h6>
                                            <form action="{{ route('products.update-image', $product->id) }}" method="POST"
                                                enctype="multipart/form-data" id="product-image-form">
                                                @csrf
                                                <div class="mb-3">
                                                    <label class="form-label text-gray-700 fw-semibold">Pilih Gambar</label>
                                                ...
                                                <div id="product-image-preview-new" class="mb-3 d-none">
                                                    <p class="text-muted mb-1 fw-semibold">Preview:</p>
                                                    <img src="" id="product-new-img" class="img-fluid rounded border"
                                                        style="max-height: 200px; object-fit: cover;">
                                                </div>
                                                <button type="submit" class="btn btn-primary btn-sm w-100">
                                                    <i class="ki-duotone ki-cloud-upload fs-4 me-1"><span
                                ...
                                <div class="d-flex flex-column h-100">
                                    <div>
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <span
                                                class="fw-bold {{ $product->stock > 0 ? 'text-success' : 'text-danger' }}">
                                                {{ $product->stock }} unit
                                            </span>
                                        </div>
                                        ...
                                        <div class="mb-4">
                                            @if($product->is_promo && $product->promo_price)
                                                ...
                                                <div class="text-success fw-bold">
                                                    Hemat Rp
                                                    {{ number_format($product->price - $product->promo_price, 0, ',', '.') }}
                                                </div>
                                            @else
                            ...
                            <div class="col-md-3">
                                <div class="card card-flush">
                                    <div class="card-header p-0">
                                        <div class="bg-light-primary rounded-top"
                                            style="height: 150px; background-image: url('{{ $related->image ?? asset('metronic/assets/media/stock/600x400/img-1.jpg') }}'); background-size: cover; background-position: center;">
                                        </div>
                                    </div>
                                    <div class="card-body">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <span class="fw-bold text-primary">Rp
                                                {{ number_format($related->getCurrentPrice(), 0, ',', '.') }}</span>
...
@section('script')
    <script>
        // Image preview function for Owner/Staff upload form
        function previewProductImage(input) {
            if (input.files && input.files[0]) {
                const reader = new FileReader();
                reader.onload = function(e) {
                    document.getElementById('product-new-img').src = e.target.result;
                    document.getElementById('product-image-preview-new').classList.remove('d-none');
                };
